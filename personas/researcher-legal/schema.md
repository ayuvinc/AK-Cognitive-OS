...
research_brief:
  question: string
  researcher: "legal"
  jurisdiction: string            # e.g. UK, US, EU, general
  date: YYYY-MM-DD
...
| Level | Meaning |
|---|---|
| high | Multiple corroborating primary legal sources (statutes, case law, official guidance); findings are well-established |
| medium | Some primary sources; some inference or secondary legal commentary involved |
| low | Limited sources; significant uncertainty; treat as preliminary research only |

## Validation Rules

- `research_brief.question` must match the original `research_question` input exactly
- `research_brief.researcher` must be `"legal"`
...
- Missing `jurisdiction` input -> use default "general / not jurisdiction-specific"
- Advisory disclaimer must be present in output
...
- `research_question` — the exact legal question to answer
- `jurisdiction` — legal jurisdiction (e.g. UK, US, EU, or "general")
